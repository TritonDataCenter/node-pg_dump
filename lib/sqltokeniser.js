...
_ingest_copy(chr)
{
	...
	var reparse = function (state) {
		sqcp.sqcp_state = state;
	...
	var commit_column = function (value, is_last) {
		if (sqcp.sqcp_output.length > sqcp.sqcp_columns.length) {
			self.sqlt_error = VE('too many columns on COPY row');
			return (INGEST_ERROR);
		}

		sqcp.sqcp_output.push(value);
		 ...
		 */
		var val = {};
		for (var i = 0; i < sqcp.sqcp_columns.length; i++) {
			val[sqcp.sqcp_columns[i]] = sqcp.sqcp_output[i];
		}
		self._commit('copy_row', val);
	...
	case 'NULL_CHECK':
		if (sqcp.sqcp_accum.length < sqcp.sqcp_null.length &&
			...
			if (chr === sqcp.sqcp_delim) {
				 ...
				 */
				return (commit_column(null, false));
			} else if (chr === '\n') {
				/*
				 * A NULL-valued column which is the last
				 * column on this line.
				 */
				return (commit_column(null, true));
			}
		}
	...
	case 'COLUMN':
		if (chr === sqcp.sqcp_delim) {
			 ...
			 */
			return (commit_column(sqcp.sqcp_accum, false));
		} else if (chr === '\n') {
			/*
			 * End of the last column in the row.
			 */
			return (commit_column(sqcp.sqcp_accum, true));
		}
	...
	case 'COLUMN_ESCAPED':
		if (chr === '.' && sqcp.sqcp_output.length === 0 &&
...
_verr_info()
{
	...
	return ({
		line_string: self.sqlt_line_string,
		line: self.sqlt_line,
